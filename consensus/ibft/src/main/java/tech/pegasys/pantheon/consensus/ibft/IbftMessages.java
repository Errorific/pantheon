  ...
  public static SignedData<?> fromMessage(final Message message) {
    ...
    switch (messageData.getCode()) {
      case IbftV2.PROPOSAL:
        return ProposalMessageData.fromMessage(messageData).decode();

      case IbftV2.PREPARE:
        return PrepareMessageData.fromMessage(messageData).decode();

      case IbftV2.COMMIT:
        return CommitMessageData.fromMessage(messageData).decode();

      case IbftV2.ROUND_CHANGE:
        return RoundChangeMessageData.fromMessage(messageData).decode();

      case IbftV2.NEW_ROUND:
        return NewRoundMessageData.fromMessage(messageData).decode();
